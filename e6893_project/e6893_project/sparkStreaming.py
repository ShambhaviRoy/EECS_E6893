...
# global variables
bucket = "sr3767-bucket-1"    # TODO : replace with your own bucket name
...
def hashtagCount(words):
    ...
    hashtag = words.map(lambda x: x.lower()).filter(
        lambda x: len(x) > 2 and x[0] == "#").map(lambda x: (x, 1))

    hashtag_count = hashtag.reduceByKey(lambda c1, c2: c1 + c2)
    hashtag_count_total = hashtag_count.updateStateByKey(updateFunc).transform(
        lambda rdd: rdd.sortBy(lambda x: x[1], ascending = False))
    

    return hashtag_count_total
...
def wordCount(words):
    ...
    # TODO: insert your code here
    word_count = words.map(lambda x: x.lower()).filter(
        lambda x: x in WORD).map(lambda x: (x,1)).reduceByKeyAndWindow(
        lambda x,y: x+y , lambda x,y: x - y, 10, 10)

    word_count_total = word_count.transform(lambda time, rdd: rdd.map(
